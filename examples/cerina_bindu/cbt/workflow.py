...
from .state import ProtocolState, create_initial_state, add_draft_version
from .agents import (
    DrafterAgent,
    SafetyGuardianAgent,
    ClinicalCriticAgent,
...
def should_continue(state: ProtocolState) -> Literal["draft", "safety_review", "clinical_critique", "supervisor", "halt", "end"]:
    ...
    print(f"DEBUG should_continue: active_agent={active_agent}, status={status}, draft_exists={bool(current_draft)}")
    
    # If halted, wait for human approval
    if state.get("halted"):
        print(f"DEBUG: Halted, returning 'end'")
        return "end"
    
    # If human approved, continue to finalization
    if state.get("human_approved"):
        state["halted"] = False
        print(f"DEBUG: Human approved, returning 'end'")
        return "end"
    
    # After drafting, always review safety
    if active_agent == "Drafter":
        print(f"DEBUG: After Drafter, routing to safety_review")
        return "safety_review"
    
    # After safety review, critique clinically
    if active_agent == "SafetyGuardian":
        print(f"DEBUG: After SafetyGuardian, routing to clinical_critique")
        return "clinical_critique"
    
    # After critique, supervisor decides
    if active_agent == "ClinicalCritic":
        print(f"DEBUG: After ClinicalCritic, routing to supervisor")
        return "supervisor"
    ...
    if active_agent == "Supervisor":
        ...
        if "needs_revision" in status:
            print(f"DEBUG: Supervisor says needs_revision, routing back to draft")
            return "draft"  # Route back to drafter for revision
        elif "ready_for_review" in status or "awaiting_review" in status:
            print(f"DEBUG: Supervisor says ready, routing to finalize")
            return "finalize"
        else:
            print(f"DEBUG: Supervisor status unclear, routing to end")
            return "end"
    
    # Default: if no draft exists, start with draft
    if not state.get("current_draft"):
        print(f"DEBUG: No draft exists, routing to draft")
        return "draft"
    
    # Default: supervisor decides
    print(f"DEBUG: Default routing to supervisor")
    return "supervisor"

def init_state(state: ProtocolState) -> ProtocolState:
    print(f"\n{'='*60}")
    print(f"INIT_STATE CALLED")
