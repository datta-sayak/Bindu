...
from .state import ProtocolState, add_agent_note, add_draft_version
...
def get_llm():
    ...
    if not api_key:
        raise ValueError("OPENAI_API_KEY not found. Get your key from https://platform.openai.com/api-keys")
...
class DrafterAgent:
    ...
    async def draft(self, state: ProtocolState) -> Dict[str, Any]:
        ...
        recent_notes = [n for n in state["agent_notes"][-5:] if n.get("target_agent") == "Drafter"]
        feedback_context = "\n".join([f"- {n['note']}" for n in recent_notes])
        
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a clinical psychologist specializing in Cognitive Behavioral Therapy (CBT).
...
Format your response as a complete CBT exercise protocol."""),
            ("human", """Create a CBT exercise for: {intent}
...
{iteration_context}""")
        ...
        if messages:
            print(f"DEBUG: Human message content preview: {str(messages[-1].content)[:150]}...")
        
        response = await self.llm.ainvoke(messages)
        draft_content = response.content
        
        print(f"DEBUG: LLM response received, length: {len(draft_content) if draft_content else 0}")
        print(f"DEBUG: Draft content preview: {draft_content[:100] if draft_content else '[empty]'}...")
...
class SafetyGuardianAgent:
    ...
    async def review(self, state: ProtocolState) -> Dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a safety reviewer for clinical content.
...
{{
    ...
    "recommendations": ["how to fix"]
        ...
        safety_checks["self_harm"] = "self-harm" not in response.content.lower() and "suicide" not in response.content.lower()
...
class ClinicalCriticAgent:
    ...
    async def critique(self, state: ProtocolState) -> Dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a senior clinical psychologist reviewing CBT exercises.
Evaluate:
...
class SupervisorAgent:
    ...
    async def decide(self, state: ProtocolState) -> Dict[str, Any]:
        ...
        quality_ok = (
            safety_score is not None and safety_score >= 0.8 and
            empathy_score is not None and empathy_score >= 0.7 and
            clinical_score is not None and clinical_score >= 0.7
        )
        ...
        needs_revision = (
            safety_score is not None and safety_score < 0.8 or
            empathy_score is not None and empathy_score < 0.7 or
            clinical_score is not None and clinical_score < 0.7
        )
