...
async def handler(messages: list[dict]):
    ...
    print(f"\n{'='*80}")
    print(f"HANDLER ENTRY: Received new request")
    ...
    try:
        ...
        print(f"HANDLER: build_langgraph_input result:")
        ...
        logger.debug(f"DEBUG: Available keys in final_state: {list(final_state.keys())}")
        logger.debug(f"DEBUG: current_draft value: {repr(final_state.get('current_draft'))}")
        logger.debug(f"DEBUG: final_response value: {repr(final_state.get('final_response'))}")
        ...
        final_response = final_state.get('final_response') or final_state.get('current_draft') or ''
        if final_response is None:
            final_response = ''
        
        # Additional logging for debugging
        if not final_response:
            logger.warning(f"WARNING: No final_response or current_draft found in state")
            logger.debug(f"DEBUG: Full state: {final_state}")
        
        final_response_preview = str(final_response)[:100] if final_response else '[empty response]'
        ...
        logger.error("🔥 HANDLER EXCEPTION — THIS SHOULD PRINT IF WE CRASH", exc_info=True)
